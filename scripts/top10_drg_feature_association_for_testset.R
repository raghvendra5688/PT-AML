...
#PREDICTIONS from Catboost model on test set
predictions <- read.csv("Results/Catboost_MFP_Feat_Var_supervised_test_predictions.csv", sep = "\t")
drug_smile <- read.csv("Data/Drug_Full_SMILES_Embedding.csv")

#Perform join on predictions to add inhibitor name and inhibitor CID
...
zip_path <- "Data/Test_Set_Var_with_Drug_MFP_Cell_Info.csv.zip"
...
#Get patient features by performing merge between cid dataframe and test dataset 
...
all_col_names <- colnames(filtered_df)
...
significant_results <- significant_results[order(significant_results$r_squared,decreasing = T),]

# Save results to CSV
write.csv(significant_results, "Results/top10_drug_features_asso/Venetoclax_LR_significant_features_for_test_set.csv", row.names = FALSE)
...
for (drug in target_inhibitors) {
  ...
  write.csv(significant_results, paste0("Results/top10_drug_features_asso/",drug, "_LR_significant_features_for_test_set.csv"), row.names = FALSE)
  
  # Store top 10 features by q-value
  top_100_features <- significant_results[order(significant_results$q_value), ][1:100, "feature"]
  all_significant_results[[drug]] <- top_100_features
  
  # Print top 10 features for the current drug
  cat("\nTop 100 Features for", drug, ":\n")
  print(top_100_features)
}

# Initialize a vector to store all top 10 features
all_top_100_features <- unlist(all_significant_results)

# Create a table of feature counts
feature_counts <- table(all_top_100_features)

# Convert to a data frame for easier viewing
feature_count_df <- as.data.frame(feature_counts)
colnames(feature_count_df) <- c("CommonFeature", "Drugs")

# Filter features that appear in at least 5 drug's top 100
feature_count_df <- feature_count_df[order(-feature_count_df$Drugs), ]
subset_feature_count_df <- feature_count_df[feature_count_df$Drugs>=5,]

# Print the result
print(subset_feature_count_df)

# Optionally, save the result to a CSV file
write.csv(subset_feature_count_df, "Results/top10_drug_features_asso/feature_counts_across_drugs.csv", row.names = FALSE)
...
for (drug in target_inhibitors) {
  ...
  drug_significant_results <- read.csv(paste0("Results/top10_drug_features_asso/",drug, "_LR_significant_features_for_test_set.csv"))
...
# Optionally, save the result to a CSV file
write.csv(feature_count_df_all_drugs, "Results/top10_drug_features_asso/feature_counts_across_all_drugs.csv", row.names = FALSE)

# Load the list of features to extract betas from the feature_counts_across_all_drugs.csv
feature_counts <- read.csv("Results/top10_drug_features_asso/feature_counts_across_all_drugs.csv")
...
write.csv(beta_matrix, "Results/top10_drug_features_asso/betas_for_selected_features_across_drugs.csv", row.names = FALSE)

#Order the beta matrix by mean absolute values (not considering NAs)
rev_beta_matrix <- beta_matrix[order(rowMeans(abs(beta_matrix[,2:ncol(beta_matrix)]),na.rm = F), decreasing = T),]

#Make a function to perform ggplot for a given colname, all drugs and AUC
make_lm_plot <- function(filtered_df, column){
  
  revised_df <- filtered_df[,c("inhibitor","predictions",column)]
  revised_df$predictions <- revised_df$predictions/300
  column <- sym(column)
  
  revised_df %>% 
    ggplot(aes(x=!!column, y=predictions, color = inhibitor, group = inhibitor)) +
    geom_point() + xlim(0,0.75)+ylim(0,1)+ xlab("Immunogenic Cell Death (ICD) Activity") + ylab("Normalized Predicted AUC")+
    geom_smooth(method = "lm", lwd=1, lty=1, color="black") +
    facet_wrap(~ inhibitor, ncol=5) + stat_cor(label.y = 0.95) + 
    theme_classic2() + 
    theme(legend.position = "none") -> g_plot
  
  g_plot <- g_plot + theme(axis.text=element_text(size=12, face="bold"),
