...
2. `top10_drg_feature_association.R`: There are 2 scripts one for train and one for test set to check the top 10 drugs association with features. The results are stored by drugname_LR_significant_features. the traingset script also have the code for comparision between test and training set.
